...
By leveraging a reasoning loop, the agent can determine which tools to utilize based on the input task, the sequence of tool calls, and the parameters required for each tool. This allows for a more nuanced interaction with data, enabling the agent to not only "read" from static sources but also dynamically ingest and modify data through external service APIs.

The Cortado AI Agent is designed to extend its functionality with an ad hoc toolset, allowing for specific developments tailored to user needs. It can process responses from various APIs and store relevant information for future use, making it a powerful knowledge worker within the LlamaIndex framework.
